  ...
  return (
    ...
    >
      <div className="flex items-center flex-grow gap-2">
        ...
        {isChild && (
          <span className="ml-1 inline-flex h-3 w-3 rounded-full bg-blue-500"></span>
        )}
      </div>
      <button
        ...
        aria-label={isPinned ? `Открепить ${item.name}` : `Прикрепить ${item.name}`}
        title={isPinned ? `Открепить ${item.name}` : `Прикрепить ${item.name}`}
        onClick={handlePinClick}
      >
        {isPinned ? <FaTimes size={14} /> : <FaThumbtack size={14} />}
      </button>
    </li>
  );
};
